...
export default function PayPremium() {
  ...
  const fetchActiveEnrollment = async () => {
    ...
    try {
      ...
      const enrollRes = await API.get("/enrollment/my-policy")
      const active: ActiveEnrollment = enrollRes.data
      setEnrollment(active)

      // Get policy details to show name & coverage
      const policyRes = await API.get(`/policy/${active.policy_id}`)
      setPolicy(policyRes.data)
    } catch (err: any) {
      const status = err?.response?.status
      if (status === 404) {
        setFetchError("No active enrollment found. Please enroll in a policy first.")
      } else {
        setFetchError("Failed to load enrollment details. Please try again.")
      }
    } finally {
      setFetching(false)
    }
  ...
  const payPremium = async () => {
    ...
    setLoading(true)
    try {
      await API.post("/premium/pay", {
        policy_id: enrollment.policy_id,
        amount:    enrollment.weekly_premium_paid,
      })
  ...
  const tier    = (enrollment?.policy_tier || "standard").toLowerCase()
  const tColor  = tierColor[tier] || "#f97316"
  ...
  return (
    <>
      <style>{`
        ...
        /* Loading */
        .pp-fetching { display:flex;align-items:center;justify-content:center;gap:12px;padding:60px 0;color:#475569;font-size:14px; }
        .pp-spin-lg { width:22px;height:22px;border:2px solid rgba(249,115,22,.2);border-top-color:#f97316;border-radius:50%;animation:ppSpin .7s linear infinite; }

        /* Error */
        .pp-error { background:rgba(239,68,68,.06);border:1px solid rgba(239,68,68,.2);border-radius:14px;padding:24px;text-align:center; }
        .pp-error-icon { font-size:32px;margin-bottom:12px; }
        .pp-error-msg { font-size:14px;color:#f87171;margin-bottom:20px; }
        .pp-error-btn { padding:10px 24px;border-radius:9px;background:linear-gradient(135deg,#f97316,#ea580c);color:#fff;border:none;font-family:'DM Sans',sans-serif;font-size:13px;font-weight:600;cursor:pointer; }

        /* Policy summary card */
        .pp-summary { background:rgba(255,255,255,.025);border:1px solid rgba(255,255,255,.07);border-radius:16px;padding:20px 22px;margin-bottom:20px;animation:ppFadeUp .4s ease both; }
        .pp-summary-header { display:flex;align-items:center;gap:12px;margin-bottom:16px; }
        .pp-summary-icon { font-size:28px; }
        .pp-summary-name { font-family:'Bebas Neue',sans-serif;font-size:22px;letter-spacing:.04em; }
        .pp-summary-tier { font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;padding:3px 10px;border-radius:100px; }
        .pp-summary-rows { display:flex;flex-direction:column;gap:8px; }
        .pp-summary-row { display:flex;justify-content:space-between;font-size:13px; }
        .pp-summary-key { color:#475569; }
        .pp-summary-val { color:#94a3b8;font-weight:500;text-transform:capitalize; }
        .pp-summary-divider { border:none;border-top:1px solid rgba(255,255,255,.06);margin:10px 0; }
        .pp-summary-total-row { display:flex;justify-content:space-between;align-items:baseline; }
        .pp-summary-total-key { font-size:14px;font-weight:600;color:#e8eaf0; }
        .pp-summary-total-val { font-family:'Bebas Neue',sans-serif;font-size:36px;letter-spacing:.03em; }

        /* Method selector */
        .pp-label { display:block;font-size:11px;font-weight:600;letter-spacing:.12em;text-transform:uppercase;color:#475569;margin-bottom:10px; }
        .pp-methods { display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:24px; }
        ...
        /* Back link */
        .pp-back { display:inline-flex;align-items:center;gap:6px;font-size:13px;color:#475569;cursor:pointer;margin-bottom:24px;transition:color .2s; }
        .pp-back:hover { color:#94a3b8; }
      ...
      <div className="pp-root">
        ...
        <div className="pp-inner">
          ...
          /* ── LOADING ── */
            ...
            <div className="pp-fetching">
              <span className="pp-spin-lg" />
              Loading your policy details…
            </div>
          ...
          /* ── ERROR / NO ENROLLMENT ── */
            ...
            <div className="pp-error">
              ...
              <div className="pp-error-msg">{fetchError}</div>
              <button className="pp-error-btn" onClick={() => router.push("/enroll-policy")}>
                Enroll in a Policy →
              </button>
            </div>
          ...
          /* ── MAIN PAYMENT FORM ── */
            ...
            <>
              {/* Back link */}
              <div className="pp-back" onClick={() => router.push("/enroll-policy")}>
                ← Change plan
              </div>

              {/* Auto-filled policy summary */}
              <div className="pp-summary">
                <div className="pp-summary-header">
                  <span className="pp-summary-icon">{tIcon}</span>
                  <div style={{ flex:1 }}>
                    <div className="pp-summary-name" style={{ color: tColor }}>
                      {policy?.policy_name || `Policy #${enrollment.policy_id}`}
                    </div>
                  </div>
                  <span
                    className="pp-summary-tier"
                ...
                <div className="pp-summary-rows">
                  <div className="pp-summary-row">
                    <span className="pp-summary-key">Policy ID</span>
                    <span className="pp-summary-val">#{enrollment.policy_id}</span>
                  </div>
                  <div className="pp-summary-row">
                    <span className="pp-summary-key">Coverage amount</span>
                    <span className="pp-summary-val">
                      ₹{(policy?.coverage_amount || 0).toLocaleString()}
                    </span>
                  </div>
                  <div className="pp-summary-row">
                    <span className="pp-summary-key">Zone</span>
                    <span className="pp-summary-val">
                      {enrollment.home_zone?.replace(/_/g, " ") || "—"}
                    </span>
                  </div>
                  <div className="pp-summary-row">
                    <span className="pp-summary-key">Coverage period</span>
                    <span className="pp-summary-val">7 days (current week)</span>
                  </div>
                  <div className="pp-summary-row">
                    <span className="pp-summary-key">Payment method</span>
                    <span className="pp-summary-val">{method}</span>
                  </div>
                </div>

                <hr className="pp-summary-divider" />

                <div className="pp-summary-total-row">
                  <span className="pp-summary-total-key">Total due</span>
                  <span className="pp-summary-total-val" style={{ color: tColor }}>
                    ₹{enrollment.weekly_premium_paid}
                  </span>
                </div>
              </div>
              ...
              <button className="pp-btn" onClick={payPremium} disabled={loading}>
                {loading
                  ...
                  : <>Pay ₹{enrollment.weekly_premium_paid} via {method} →</>
                }
              </button>
            </>
          )}
